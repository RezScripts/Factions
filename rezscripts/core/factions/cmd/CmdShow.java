...
import rezscripts.core.factions.zcore.util.TagReplacer;
...
public class CmdShow extends FCommand {
    ...
    public CmdShow() {
        ...
        defaults.add("<a>Enemies(<i>{enemies}<a>): {enemies-list}");
        defaults.add("<a>Allies(<i>{allies}<a>/<i>{max-allies}<a>): {allies-list}");
        defaults.add("<a>Truces(<i>{truces}<a>/<i>{max-truces}<a>): {truce-list}");
