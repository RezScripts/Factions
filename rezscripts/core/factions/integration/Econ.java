package rezscripts.core.factions.integration;
...
import rezscripts.core.factions.util.RelationUtil;
...
//			ep.msg("<h>%s<i> didn't have to pay anything %s.", You, forDoingThis);  // might be for gains, might be for losses
    ...
    public static double calculateTotalLandValue(Faction fac) {        
        	//TODO:calculate land value
        
        return fac.getLandValue();
    }
    ...
    public static double getBalance(Player account) {
        return econ.getBalance(account);
    }
	...
	public static double calculateTotalValue(Faction fac) 
	{
		
		int ownedLand = fac.getLandRounded();
        double amount = 0;
        
        amount = amount + calculateBalanceValue(fac) + calculateTotalLandValue(fac);
        
        return amount;
	}

	public static double calculateBalanceValue(Faction fac) {
        double amount = 0;
        
        for (FPlayer fp : fac.getFPlayers()) 
        {
        	Player player = fp.getPlayer();
        	amount = amount + Econ.getBalance(player);
        }
