...
public class RegionBoundary {
    ...
    public double area() {
        ...
        for (int i = 0; i < getPoints().length - 1; i++) {
            sum = sum + (getPoints()[i].x * getPoints()[i + 1].z) - (getPoints()[i].z * getPoints()[i + 1].x);
        }
        sum = Math.abs(sum);
        sum *= 0.5;
        sum *= (ymax - ymin);
    ...
    public boolean contains(Location loc) {
        return contains((long) loc.getX(),(long)loc.getY(),(long)loc.getZ());
    }
        ...
        for (i = 0, j = getPoints().length - 1; i < getPoints().length; j = i++) {
            if ((getPoints()[i].z > z) != (getPoints()[j].z > z) && (x < (getPoints()[j].x - getPoints()[i].x) * (z - getPoints()[i].z) / (getPoints()[j].z - getPoints()[i].z) + getPoints()[i].x)) {
                result = !result;
            }
        }
        if (y < ymin || y > ymax)
