...
public class FactionsChatListener implements Listener {
    ...
    public void onPlayerEarlyChat(AsyncPlayerChatEvent event) {
        ...
        if (chat == ChatMode.FACTION) {
        ...
        else if (chat == ChatMode.ENEMY) {
            Faction myFaction = me.getFaction();

            String message = String.format(Conf.enemyChatFormat, ChatColor.stripColor(me.getNameAndTag()), msg);
            ...
            //Send to all our allies
            for (FPlayer fplayer : FPlayers.getInstance().getOnlinePlayers()) {
                if (myFaction.getRelationTo(fplayer) == Relation.ENEMY) {
                    fplayer.sendMessage(message);
                } else if (fplayer.isSpyingChat() && me != fplayer) {
                    fplayer.sendMessage("[ECspy]: " + message);
                }
            }

            Bukkit.getLogger().log(Level.INFO, ChatColor.stripColor("EnemyChat: " + message));
