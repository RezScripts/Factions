...
import static rezscripts.core.factions.zcore.util.TagReplacer.TagType;
...
public class TagUtil {
    ...
    protected static List<FancyMessage> getFancy(Faction target, FPlayer fme, TagReplacer type, String prefix) {
        ...
        switch (type) {
            ...
            case TRUCE_LIST:
                FancyMessage currenTruces = P.p.txt.parseFancy(prefix);
                ...
                for (Faction otherFaction : Factions.getInstance().getAllFactions()) {
                    ...
                    if (otherFaction.getRelationTo(target).isTruce()) {
                    	currenTruces.then(firstTruce ? s : ", " + s);
                    	currenTruces.tooltip(tipFaction(otherFaction)).color(fme.getColorTo(otherFaction));
                    	firstTruce = false;
                        if (currenTruces.toJSONString().length() > ARBITRARY_LIMIT) {
                            fancyMessages.add(currenTruces);
                            currenTruces = new FancyMessage("");
                        }
                    }
                }
                fancyMessages.add(currenTruces);
                return firstTruce && minimal ? null : fancyMessages;
            case ENEMIES_LIST:
